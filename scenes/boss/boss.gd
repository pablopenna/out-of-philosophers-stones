...
func _shoot_next_pattern() -> void:
	# _shoot_360(50, Vector2.RIGHT)
...
func _shoot_360(number_of_projectiles: int) -> void:
	...
	for n: int in number_of_projectiles:
		var direction: Vector2 = Vector2.RIGHT.rotated(direction_offset * n)
...
func _shoot_filled_cone(target_direction: Vector2) -> void:
	var number_of_bullets_in_cone = 5
	var cone_angle: float = PI / 8
	var speed: int = 100
	var damage: int = 1
	
	for i: int in range(1, number_of_bullets_in_cone+1):
		var current_bullet_angle = (cone_angle / number_of_bullets_in_cone) * i
